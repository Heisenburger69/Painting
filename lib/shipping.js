...
export function calculateFedExShipping(cartItems, governorateKey, options = {}) {
  const zone = ZONE_MAP[governorateKey]
  if (!zone || cartItems.length === 0) return { totalShipping: 0, billableWeight: 0 }
  ...
  const PAD = 3
  const finalLength = maxLength + PAD
  const finalWidth = maxWidth + PAD
  const finalHeight = totalThickness
